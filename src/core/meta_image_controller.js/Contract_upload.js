...
const POLICY_DIR = path.join(__dirname, "../../Access/Contract_template");
...
const storage = multer.diskStorage({
  ...
  filename: (req, file, cb) => {
    ...
    const newName = `CONTRACT-${Date.now()}-${Math.round(Math.random() * 1e9)}${ext}`;
    cb(null, newName);
  },
});
...
const Contract_upload = multer({
  storage,
  fileFilter,
  limits: { fileSize: 100 * 1024 * 1024 },
});

export default Contract_upload;
