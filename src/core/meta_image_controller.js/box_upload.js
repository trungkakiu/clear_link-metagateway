...
const MAIN_DIR = path.join(__dirname, "../../Access/box");
const SUB_DIR = path.join(__dirname, "../../Access/box");

const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    if (file.fieldname === "image_printer") {
      cb(null, MAIN_DIR);
    } else if (file.fieldname === "decription_image") {
      cb(null, SUB_DIR);
    } else {
...
const box_upload = multer({
  storage,
  fileFilter,
  limits: { fileSize: 15 * 1024 * 1024 },
});

export default box_upload;
