...
const POLICY_DIR = path.join(__dirname, "../../Access/certificates_competence");
...
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    if (file.fieldname === "attached_file") {
      cb(null, POLICY_DIR);
    } else {
...
const certificates_competence_upload = multer({
  storage,
  fileFilter,
  limits: { fileSize: 100 * 1024 * 1024 },
});

export default certificates_competence_upload;
